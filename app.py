...
MIN_VOL_NEAR     = 50     # 근만기(0~2DTE)
MIN_VOL_REGULAR  = 200    # 일반 만기
...
def get_volume_weighted_iv(df, is_near_expiry=False):
    try:
        ...
        mask           = (iv > IV_MIN) & (iv < IV_MAX) & (vol > 0) & liquid
...
def build_flow_df(df, option_type, is_near_expiry=False):
    min_vol = MIN_VOL_NEAR if is_near_expiry else MIN_VOL_REGULAR
    rows    = []
    for _, row in df.iterrows():
        try:
            ...
            if iv_raw >= IV_MAX:
                continue
            iv_pct = round(iv_raw * 100, 1) if IV_MIN < iv_raw < IV_MAX else None
...
# ── Max Pain 계산 ─────────────────────────────────────────────────────────────
def calc_max_pain(calls, puts, current_price=0.0, is_near_expiry=False):
    try:
        c = calls.copy()
        p = puts.copy()

        price_range      = 0.25 if is_near_expiry else 0.40
        validation_range = 0.15 if is_near_expiry else 0.30
        ...
        pain = []
        for s in strikes:
            cp = float(((s - c['strike']).clip(lower=0) * c['openInterest']).sum())
            pp = float(((p['strike'] - s).clip(lower=0) * p['openInterest']).sum())
            pain.append(cp + pp)

        result = strikes[pain.index(min(pain))]

        if current_price > 0:
            deviation = abs(result - current_price) / current_price
            if deviation > validation_range:
                return None, (
                    f"결과값(${result:.0f})이 현재가 대비 "
                    f"{deviation*100:.0f}% 괴리 → "
                    f"{'0DTE ' if is_near_expiry else ''}데이터 오염 의심"
                )
...
def validate_option_data_integrity(calls, puts, current_price):
    ...
    def check_stale_quotes(df, opt_type):
        ...
        if not active_opts.empty:
            stale_condition = (
                ((active_opts['bid'] > 0) & ((active_opts['lastPrice'] < active_opts['bid'] * 0.5) | (active_opts['lastPrice'] > active_opts['ask'] * 1.5))) |
    ...
    if current_price > 0:
        ...
        if atm_calls.empty or atm_calls['volume'].sum() == 0:
            warnings.append(f"현재가(${current_price:.2f}) 부근의 등가격(ATM) 옵션 거래 데이터가 없습니다. 기초자산 가격과 옵션 체인 기준일이 불일치할 수 있습니다.")
...
if analysis_mode == "단일 만기일 분석" and selected_expiry and expirations:
    ...
    max_pain, mp_reason = calc_max_pain(calls, puts, current_price=current_price, is_near_expiry=is_near)
    ...
    if current_price > 0:
        ...
        if cs is not None or ps is not None:
            ...
            all_iv_values = []
            if cs is not None:
                all_iv_values.extend(cs['iv_pct'].tolist())
            if ps is not None:
                all_iv_values.extend(ps['iv_pct'].tolist())
            if all_iv_values:
                iv_p95 = float(np.percentile(all_iv_values, 95))
                y_max  = min(iv_p95 * 1.2, IV_MAX * 100) 
            else:
                y_max = 100

            fig_iv.update_layout(
                title="IV Skew (비유동성 이상치 제거, Y축 P95 기준 자동 스케일)",
    ...
    if not all_flow.empty:
        ...
        st.markdown("##### 💹 실질 시장 방향성 (옵션 구조 반영)")

        def sum_premium(df, opt_type, direction):
            sub = df[(df['종류'] == opt_type) & (df['_direction'] == direction)]
            return float(sub['프리미엄($)'].sum())

        call_buy_p  = sum_premium(all_flow, 'CALL', 'BUY')
        call_sell_p = sum_premium(all_flow, 'CALL', 'SELL')
        put_buy_p   = sum_premium(all_flow, 'PUT',  'BUY')
        put_sell_p  = sum_premium(all_flow, 'PUT',  'SELL')

        bullish_p = call_buy_p + put_sell_p   
        bearish_p = call_sell_p + put_buy_p   
        total_p   = bullish_p + bearish_p

        if total_p > 0:
            bull_ratio = bullish_p / total_p
            if   bull_ratio > 0.65: bb_signal, bb_color = "강한 상승 신호", "#00e5a0"
            elif bull_ratio > 0.55: bb_signal, bb_color = "약한 상승 신호", "#4ade80"
            elif bull_ratio < 0.35: bb_signal, bb_color = "강한 하락 신호", "#ff4d6d"
            elif bull_ratio < 0.45: bb_signal, bb_color = "약한 하락 신호", "#f87171"
            else:                   bb_signal, bb_color = "균형/중립",       "#f5a623"

            bb_df = pd.DataFrame([
                {'분류': '🟢 BULLISH (CALL BUY + PUT SELL)',
                 '프리미엄': f"${bullish_p:,.0f}",
                 '비중':    f"{bull_ratio*100:.1f}%"},
                {'분류': '🔴 BEARISH (CALL SELL + PUT BUY)',
                 '프리미엄': f"${bearish_p:,.0f}",
                 '비중':    f"{(1-bull_ratio)*100:.1f}%"},
            ])
            st.dataframe(bb_df, use_container_width=True, hide_index=True)
            st.markdown(
                f"<div style='font-size:14px;padding:10px;background:#111827;"
                f"border-radius:8px;border-left:4px solid {bb_color};'>"
                f"<b>종합 판단:</b> <span style='color:{bb_color};font-weight:700;'>"
                f"{bb_signal}</span> (Bullish 비중 {bull_ratio*100:.1f}%)"
                f"</div>",
                unsafe_allow_html=True
            )
        else:
            bb_signal = "집계 불가"
            bull_ratio = 0
            st.info("Bullish/Bearish 집계할 충분한 플로우 데이터가 없습니다.")
    ...
    # ── 데이터 정합성 검증 (Cross-check 1) ──────────────────────────────
    data_warnings = validate_option_data_integrity(calls, puts, current_price)
    if data_warnings:
        warn_box("<b>⚠️ yfinance 원본 데이터 정합성 경고 (크로스체크 결과):</b><br>" + 
                 "<br>".join([f"• {w}" for w in data_warnings]))

    # ── 프롬프트 변수 무결성 크로스체크 (Cross-check 2) ────────────────
    try:
        assert int(call_vol) == int(calls['volume'].sum()), "콜 거래량 집계 불일치"
        assert int(put_vol) == int(puts['volume'].sum()), "풋 거래량 집계 불일치"
        
        calc_bullish_p = call_buy_p + put_sell_p
        calc_bearish_p = call_sell_p + put_buy_p
        assert int(bullish_p) == int(calc_bullish_p), "Bullish 프리미엄 계산 무결성 오류"
        assert int(bearish_p) == int(calc_bearish_p), "Bearish 프리미엄 계산 무결성 오류"
        
    except AssertionError as e:
        st.error(f"❌ 내부 데이터 교차 검증 실패: {e}. 분석 결과가 부정확할 수 있습니다.")
        st.stop()
...
[실질 시장 방향성 — 옵션 구조 반영 집계 (중요)]
※ 옵션 매도(SELL)는 반대 방향 배팅임을 반영한 해석:
  - CALL BUY  = 상승 배팅 (BULLISH)
  - CALL SELL = 상방 저항 / 상승 제한 (BEARISH)
  - PUT  BUY  = 하락 배팅 (BEARISH)
  - PUT  SELL = 하방 지지 / 강세 확신 (BULLISH)  ← 숨은 기관 Bullish 신호

- BULLISH 총 프리미엄 (CALL BUY + PUT SELL): ${bullish_p:,.0f}
- BEARISH 총 프리미엄 (CALL SELL + PUT BUY): ${bearish_p:,.0f}
...
4. <b>실질 시장 방향성</b> 집계 결과(BULLISH vs BEARISH 프리미엄 비중)를 PCR과 교차 검증하세요.
   - 만약 PCR은 Bullish인데 실질 방향성이 Bearish면 → 기관의 진짜 의도는 하락 배팅일 가능성
   - PUT SELL 프리미엄이 크면 → 기관이 해당 행사가를 강한 지지선으로 간주
