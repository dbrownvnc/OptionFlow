...
# ── 필터 상수 ─────────────────────────────────────────────────────────────────
# IV 상한: 기존 5.0(500%) → 2.0(200%)로 하향
#   이유: 300%+ IV는 Deep OTM의 Black-Scholes 역산 실패로 인한 계산 오류가 대부분.
#   실제 극단 상황(테슬라 earnings 직전 등)도 150% 수준. 200% 넘으면 거의 이상치.
IV_MIN = 0.01    # 1%
IV_MAX = 2.00    # 200%

# Smart Money 최소 거래량: 기존 10/100 → 50/200 강화
#   이유: 거래량 14건짜리가 "Smart Money"로 분류되는 것은 의미 없음.
#   스마트 머니는 의미 있는 체결량이 있어야 함.
...
def build_flow_df(df, option_type, is_near_expiry=False):
    ...
    for _, row in df.iterrows():
        try:
            ...
            # ★ IV 이상치 행은 아예 제외 (IV 300%+ 같은 오염된 데이터 배제)
...
#
# [수정 내용]
#   1. current_price ±PRICE_RANGE(0DTE는 25%, 그 외 40%) 이내 행사가만 사용
#   2. 필터 후에도 OI=0이면 None 반환
#   3. 결과가 0DTE에서 ±15%, 일반에서 ±30% 벗어나면 "데이터 오염" 처리
# -----------------------------------------------------------------------------
...
if analysis_mode == "단일 만기일 분석" and selected_expiry and expirations:
    ...
    if current_price > 0:
        ...
        if cs is not None or ps is not None:
            ...
            # ★ Y축 자동 스케일: 95%분위수 기준 + 20% 여유로 극단값 차단
            all_iv_values = []
            if cs is not None:
            ...
            if all_iv_values:
                ...
                y_max  = min(iv_p95 * 1.2, IV_MAX * 100)  # 상한 200% 캡
            else:
                y_max = 100
    ...
    if not all_flow.empty:
        ...
        # ── ★ Bullish/Bearish 집계 (옵션 유형 × 방향 조합) ────────────────────
        #   CALL BUY  = 상승 배팅 (BULLISH)
        #   CALL SELL = 상승 제한 / 저항 (BEARISH)
        #   PUT BUY   = 하락 배팅 (BEARISH)
        #   PUT SELL  = 하방 지지 (BULLISH)  ← 기관의 숨은 Bullish 신호
        ...
        bullish_p = call_buy_p + put_sell_p   # 상승 배팅 + 하방 지지
        bearish_p = call_sell_p + put_buy_p   # 상방 저항 + 하락 배팅
